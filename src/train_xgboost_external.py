#!/usr/bin/env python3
"""
train_xgboost_external.py - Train XGBoost model with external features
Purpose: Train and evaluate XGBoost using USD/IDR and external market data
"""
...
def main():
    ...
    model_output = Path("models/xgboost_usd_idr_external.pkl")
    predictions_output = Path("data/processed/xgboost_external_predictions.csv")
    ...
    print("="*60)
    print("USD/IDR XGBoost Model Training (External Features)")
    print("="*60)
    ...
    print(f"\n" + "="*60)
    print("XGBOOST MODEL RESULTS (WITH EXTERNAL FEATURES)")
    print("="*60)
    ...
    # Check if previous predictions exist
    prev_predictions_file = Path("data/processed/xgboost_predictions.csv")
    
    if prev_predictions_file.exists():
        try:
            prev_df = pd.read_csv(prev_predictions_file)
            prev_mae = prev_df['Abs_Error'].mean()
            prev_mape = prev_df['Pct_Error'].mean()
            
            print(f"\n   Previous XGBoost (without external):")
            print(f"   - MAE: {prev_mae:,.2f} IDR")
            print(f"   - MAPE: {prev_mape:.2f}%")
            
            print(f"\n   Current XGBoost (with external):")
            print(f"   - MAE: {mae:,.2f} IDR")
            print(f"   - MAPE: {mape:.2f}%")
            
            mae_diff = mae - prev_mae
            mape_diff = mape - prev_mape
            
            print(f"\n   Improvement:")
            print(f"   - MAE difference: {mae_diff:+,.2f} IDR ({'worse' if mae_diff > 0 else 'better'})")
            print(f"   - MAPE difference: {mape_diff:+.2f}% ({'worse' if mape_diff > 0 else 'better'})")
        except Exception as e:
            print(f"   ⚠ Could not load previous predictions: {e}")
    else:
        print(f"   ⚠ Previous predictions not found")
    ...
    print("\n" + "="*60)
    print("XGBoost External Training Complete!")
    print("="*60)
    print("""
Next steps:
- Run python src/compare_models.py to compare all models
- Run python src/visualize_external_result.py for visualization
""")
